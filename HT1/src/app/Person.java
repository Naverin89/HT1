...
public class Person {
	...
	public boolean validateFMLNamePart(String fml_name_part, boolean empty_allowed)
	{
	    if (empty_allowed)
	    {
	    	Matcher matcher = Pattern.compile("[А-я\\w-]{0,150}").matcher(fml_name_part);
	    	return matcher.matches();
	    }
	    else
	    {
	    	Matcher matcher = Pattern.compile("[А-я\\w-]{1,150}").matcher(fml_name_part);
	    	return matcher.matches();
	    }
	    
	}
	
	// Валидация номера телефона (допускаються только цифры, + - #)
