...
<head>
    ...
    <title>Управление номерами телефона</title>
</head>
<body>
<%
    ...
    %>
    <tr>
        <td colspan="2">Информация о владельцe телефона:
            <%=" " + person.getSurname() + " " + person.getName() + " " + person.getMiddlename()%>
        </td>
    </tr>
    <tr>
    <td>Номер:</td>
        <form action="${pageContext.request.contextPath}/ManagePersonsPhones/" method="post">
        <%
            if (!phoneId.isEmpty()) {
        %>
        
            <td>
                <input type="text" name="phone" value="<%=person.getPhones().get(phoneId)%>"/>
                <input type="text" name="phoneId" hidden="true" value="<%=phoneId%>"/>
 	...
 	<tr>
        <td colspan="2" align="center">
            ...
            <a href="${pageContext.request.contextPath}/ManagePerson/?action=edit&id=<%=person.getId()%>&phoneId=<%=phoneId%>">Вернуться к
                данным о
                человеке</a>
        </td>
    </tr>
